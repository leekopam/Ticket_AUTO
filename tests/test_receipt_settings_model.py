...
class ReceiptSettingsModelTest(unittest.TestCase):
    ...
    def test_managed_sound_paths_are_saved_as_portable_relative_paths(self) -> None:
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            managed_sound = root / "Resources" / "sound" / "added.wav"
            managed_sound.parent.mkdir(parents=True, exist_ok=True)
            managed_sound.write_bytes(b"wav")

            settings = ReceiptSettings(
                qr_scan_success_sound_path=str(managed_sound),
                qr_scan_success_sound_rules=[
                    ScanSuccessSoundRule(name="추가", sound_path=str(managed_sound)),
                ],
            )
            ...
            self.assertEqual(payload["qr_scan_success_sound_path"], "Resources/sound/added.wav")
            self.assertEqual(
                payload["qr_scan_success_sound_rules"][0]["sound_path"],
                "Resources/sound/added.wav",
            )
