...
def _copy_scan_sound_file_to_resources(src_path: str) -> str:
    ...
    return make_project_relative_path(RESOURCE_SOUND_DIR / src.name)
...
def _open_path_in_explorer(raw_path: str) -> bool:
    ...
    target = resolve_runtime_file_path(path_text)
    try:
        if target.exists():
            if target.is_file():
    ...
    sound_settings_card = ft.Container(
        ...
        content=ft.Column(
            controls=[
                ...
                ft.Text("스캔 성공 시 재생할 MP3 또는 WAV 파일을 추가합니다.", size=12, color="#64748B"),
                *(
            ...
            controls=[
                ...
                ft.Container(
                    ...
                    content=ft.Column(
                        controls=[
                            ft.Text("QR 스캔 완료 알림음", size=18, weight=ft.FontWeight.BOLD),
                            ft.Text("추가한 MP3 또는 WAV 파일이 수령 처리 성공 직후 재생됩니다.", size=12, color="#666666"),
                            *(
    ...
    *,
    ...
    def on_preview_scan_sound(_: ft.ControlEvent) -> None:
        ...
        if audio_svc.play_file(path):
            ticket_settings_status_text.value = f"미리 듣기 재생: {resolve_runtime_file_path(path).name}"
        else:
...
def build_app_settings_panel(
    ...
    *,
    ...
    btn_pick_sound = ft.ElevatedButton("음원 추가", icon=ICONS.AUDIO_FILE_ROUNDED)
    ...
    def _handle_sound_files(files: list[ft.FilePickerFile]) -> None:
        ...
        for file in files:
            path = file.path
            if not path:
                settings_status_text.value = "음원 추가 실패: 파일 경로를 읽을 수 없습니다."
                page.update()
                return
            try:
    ...
    def on_preview_sound(_: ft.ControlEvent) -> None:
        ...
        if not path:
            ...
            return
        if audio_svc.play_file(path):
            settings_status_text.value = f"미리 듣기 재생: {resolve_runtime_file_path(path).name}"
        else:
